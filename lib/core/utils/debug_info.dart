...
class DebugInfo {
  ...
  static Future<Map<String, dynamic>> getDebugInfo() async {
    ...
    return {
      ...
      'network': {
        ...
        'recommendedInterval':
            connectionMonitor.recommendedUpdateInterval.inMilliseconds,
      },
      'battery': {
  ...
  static Future<String> getDebugString() async {
    ...
    buffer.writeln(
      '  Emulator: ${info['device']['isAndroidEmulator'] || info['device']['isIOSSimulator']}',
    );
    buffer.writeln();
    ...
    buffer.writeln(
      '  Quality: ${info['network']['quality']} (${info['network']['averageLatency']}ms)',
    );
    ...
    buffer.writeln(
      '  Hardware Accel: ${info['device']['supportsHardwareAccel']}',
    );
  ...
  static Future<Map<String, dynamic>> testConnectivity() async {
    ...
    try {
      ...
      results['socketTest'] = {'success': false, 'error': e.toString()};
    }
    ...
    try {
      ...
      final request = await client.getUrl(
        Uri.parse('http://10.0.2.2:8081/health'),
      );
      ...
      results['httpTest'] = {'success': false, 'error': e.toString()};
    }
