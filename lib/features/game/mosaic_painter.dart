...
class MosaicPainter extends CustomPainter {
  ...
  void _drawTile(Canvas canvas, Tile tile, double tileSize) {
    ...
    if (tile.isClaimed && tile.claimIntensity > 0) {
      final overlayPaint = Paint()
        ..color = Colors.white.withValues(alpha: tile.claimIntensity * 0.3)
        ..style = PaintingStyle.fill;
  ...
  void _drawPattern(Canvas canvas, Rect rect, String pattern) {
    ...
    switch (pattern) {
      case 'diagonal':
        canvas.drawLine(rect.topLeft, rect.bottomRight, patternPaint);
        break;
      case 'cross':
        canvas.drawLine(rect.topLeft, rect.bottomRight, patternPaint);
        canvas.drawLine(rect.topRight, rect.bottomLeft, patternPaint);
        break;
      case 'dots':
